 ...
 - (void)loadView
{
 ...
 //   MenuCallView *menuView = [[MenuCallView alloc] initWithFrame: CGRectMake(65.0f, 52.0f, 285.0f, 300.0f)];
  [menuView setDelegate:self];
  [menuView setTitle:NSLocalizedString(@"Mute", @"Call View")
                image:[UIImage imageNamed:@"mute.png"] forPosition:0];
  [menuView setTitle:NSLocalizedString(@"Dialpad", @"Call View")
                image:[UIImage imageNamed:@"dialer.png"] forPosition:2];  //1
  [menuView setTitle:NSLocalizedString(@"Speaker", @"Call View")
                image:[UIImage imageNamed:@"speaker.png"] forPosition:3]; /*2 */
  //[menuView setTitle:NSLocalizedString(@"add call", @"Call View")
                //image:[UIImage imageNamed:@"addcall.png"] forPosition:3];
#if HOLD_ON
  [menuView setTitle:NSLocalizedString(@"Hold", @"Call View")
                image:[UIImage imageNamed:@"hold.png"] forPosition:5];  //4
#endif
...
- (void)viewWillAppear:(BOOL)animated
{
  [super viewWillAppear:animated];
    ...
    prefxnmbr = [[NSUserDefaults standardUserDefaults]stringForKey:@"PREFIXNUMBER"];
    prefxnmbrNew = [[NSUserDefaults standardUserDefaults]stringForKey:@"PREFIXNUMBERNEW"];
...
- (void)phonePad:(id)phonepad keyDown:(char)car
{
    ...
    NSString *curText = [_lcd returnText];
    [_lcd setText: [curText stringByAppendingString: [NSString stringWithFormat:@"%c",car]]];
  ...
  switch(state)
  {
    case PJSIP_INV_STATE_CALLING: // After INVITE is sent.
      ...
      if (_call[call_id] == nil)
      {
        ...
        //[_lcd setText: [_call[call_id] displayName]];
           NSString *numberStr = [_call[call_id] displayName];
          ///////////////////////////////////////////
          NSCharacterSet *notAllowedChars = [[NSCharacterSet characterSetWithCharactersInString:@"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"] invertedSet];
          NSRange  range = [numberStr rangeOfCharacterFromSet:notAllowedChars];
          if (NSNotFound != range.location) {
              
              if(numberStr.length>8)
              {
                  NSString *numStr = [numberStr substringFromIndex:8];
                  ...
                  {
                      [_lcd setText: numberStr];
                      NSLog(@"Name2 %@",numberStr);
                  }
        ...
        [_lcd setSubImage:[self findImageWithRecordID:_call[call_id].uid]];
          ...
          [_lcd setBackgroundColor:[UIColor blackColor]];
    ...
    case PJSIP_INV_STATE_INCOMING: // After INVITE is received.
      ...
      if (_call[call_id] == nil)
      {
       ...
       [_lcd setLabel: NSLocalizedString(@"Incoming...", @"Call view")];
    ...
    case PJSIP_INV_STATE_DISCONNECTED:
      ...
      [[menuView buttonAtPosition:3] setSelected:NO];
#if HOLD_ON
      [[menuView buttonAtPosition:5] setSelected:NO];
#endif
#endif
      break;
  }  
