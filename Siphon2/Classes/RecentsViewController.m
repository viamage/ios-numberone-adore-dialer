...
NSString *prefxNmr;
NSString *prefxNmrNew;
NSString *key8 = @"hdjdtie36457jdit";
...
#else
    self.tabBarItem.title = NSLocalizedString(@"", @"Recents View");
...
- (void)viewDidLoad 
{
    ...
    prefxNmr = [[NSUserDefaults standardUserDefaults]stringForKey:@"PREFIXNUMBER"];
    prefxNmrNew = [[NSUserDefaults standardUserDefaults]stringForKey:@"PREFIXNUMBERNEW"];
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath 
{
  ...
  //label.text = [call displayName];
    /////////////////////////////////////////////
    NSString *strNumber  =    [call displayName];
    
    NSCharacterSet *notAllowedChars = [[NSCharacterSet characterSetWithCharactersInString:@"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"] invertedSet];
    NSRange  range = [strNumber rangeOfCharacterFromSet:notAllowedChars];
    if (NSNotFound != range.location) {
        
        if(strNumber.length>8)
        {
            NSString *numStr = [strNumber substringFromIndex:8];
            if([numStr hasPrefix:@"00"])
            {
                label.text = numStr;
                NSLog(@"Name1 %@",numStr);
            }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    al= indexPath.row;
    // [self.tableView reloadData];
    UIAlertView *alert1=[[UIAlertView alloc]initWithTitle:@"Make a Choice:" message:@"" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"via Internet(VOIP)", @"Via Access Number(DID)", Nil];
    
    // [alert1 setTag:indexPath.row];
    [alert1 setTag:101];
    [alert1 show];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    //[self.tableView reloadData];
    //PhoneViewController *ph=[[ PhoneViewController alloc]init];

    
    RecentCall *call = [calls objectAtIndex:al];
    NSString *phn=[call number];
    NSLog(@"phone number=%@",phn);
    if (phn!=nil) {
        
        NSString *str=[call compositeName];
        [[NSUserDefaults standardUserDefaults]setObject:str forKey:@"recents"];
        
    }
    NSString *Did=[[NSUserDefaults standardUserDefaults]valueForKey:@"didNumber"];
    NSString *text1=@"tel:";
    
    if(alertView.tag==101)
    {
        
        if(buttonIndex==1)//sip call in recent
        {
        ...
        if(buttonIndex==2)//local call
        {
            
             NSString *Did=[[NSUserDefaults standardUserDefaults]valueForKey:@"didNumber"];
            
            UIAlertView *alert2=[[UIAlertView alloc]initWithTitle:@"Choose DID" message:@"" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:Did, nil];
            [alert2 setTag:7001];
            [alert2 show];
        ...
        
    {
        ...
        if(buttonIndex==1)
        {
            ...
            // label = (UILabel *)[cell.contentView viewWithTag:NAME_TAG];
            //  NSString *check = [calls displayName];
            
            text1=@"tel:";
            
            text1= [text1 stringByAppendingString:Did];
           ...
           // text1= [text1 stringByAppendingString:@"%23"];
            ...
            text1= [text1 stringByAppendingString:phn];
            text1= [text1 stringByAppendingString:@"%23"];
...
- (void)createEditableCopyOfDatabaseIfNeeded 
{
  ...
  NSString *documentsDirectory = [NSString stringWithFormat:@"%@/Adore SIP Client", [paths objectAtIndex:0]];
#else
