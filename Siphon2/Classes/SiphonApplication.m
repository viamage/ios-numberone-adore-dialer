...
static NSString *kVoipOverEdge = @"Adore SIP ClientOverEDGE";
...
//                                                                         "and then tap the \"Globus Telecom\" entry.", @"Dilsay SIP Client")];
//    
    ...
    UIAlertView *alert = [[[UIAlertView alloc] initWithTitle:nil
                                                    ...
                                                    delegate:nil
#endif
                                           cancelButtonTitle:NSLocalizedString(@"Cancel", @"Dilsay SIP Client")
                                           otherButtonTitles:NSLocalizedString(@"", @"Dilsay SIP Client"), nil ] autorelease];
   // [alert show];
    //[alert release];
}
...
-(void)displayError:(NSString *)error withTitle:(NSString *)title
{
    UIAlertView *alert = [[[UIAlertView alloc] initWithTitle:title
                                                     message:error
                                                    delegate:nil
                                           cancelButtonTitle:NSLocalizedString(@"OK", @"Dilsay SIP Client")
...
- (void)keepAwakeEnabled
{
    ...
    [super addStatusBarImageNamed:@"Dilsay SIP Client" removeOnExit: YES];
}
...
- (void)keepAwakeDisabled
{
    ...
    [self removeStatusBarImageNamed:@"Dilsay SIP Client"];
...
- (UITabBarController*)applicationStartWithoutSettings
{
    ...
    /* Contacts */
    moreViewController *moreViewCtrl = [[[moreViewController alloc]
                                                init] autorelease];
    moreViewCtrl.phoneCallDelegate = self;
    
    ////////////**************************  creating account page in application ****************//////////////////////////////////////////
  ...
  /*  tabBarController = [[UITabBarController alloc] init];
    tabBarController.viewControllers = [NSArray arrayWithObjects:recentsViewCtrl, phoneViewController,contactsViewCtrl,acountviewcontroller, nil];
    ...
    [[UITabBar appearance] setBarTintColor:[UIColor colorWithRed: 0.50 green:0.06 blue:0.05 alpha:1.0]];
...
/*
 ...
 text.text = NSLocalizedString(@"Dilsay SIP Client requires a valid\nSIP account.\n\nTo enter this information, select \"Settings\" from your Home screen, and then tap the \"Dilsay SIP Client\" entry.", @"Dilsay SIP Client");
 ...
 text.text = NSLocalizedString(@"Press the Home button", @"Dilsay SIP Client");
 [mainView addSubview:text];
...
- (UITabBarController *)applicationStartWithSettings
{
        /* Recents list */
    recentsViewController = [[RecentsViewController alloc]
                             initWithStyle:UITableViewStylePlain];
   ...
   acountviewcontroller.phoneCallDelegate=self;
     ...
     */
    /*tabBarController = [[UITabBarController alloc] init];
    tabBarController.viewControllers = [NSArray arrayWithObjects:/* phoneViewController,*///recentsViewCtrl, //contactsViewCtrl,acountviewcontroller, nil];
    //tabBarController.selectedIndex = 0;
    
    //return tabBarController.view;*/
    tabBarController = [[UITabBarController alloc] init];
    tabBarController.viewControllers = [NSArray arrayWithObjects:
                                        /*favoritesViewCtrl*/ phoneViewController,
                                        recentsViewCtrl,moreViewCtrl
                                        /*voicemailNavCtrl*/, nil];
    tabBarController.selectedIndex = 0;
...
- (void)applicationDidFinishLaunching:(UIApplication *)application
{
    ...
    NSString *libraryDirectory = [NSString stringWithFormat:@"%@/Dilsay SIP Client", [paths objectAtIndex:0]];
	...
	//	!"sip.yelloworchidphone.com")//[[userDef objectForKey: @"server"] length])
    
    if (![[userDef objectForKey: @"username"] length] ||
        ![[userDef objectForKey: @"server"] length])
    {
        [window setRootViewController: [self applicationStartWithoutSettings]];
    ...
    {
        //  NSString *server = @"sip.yelloworchidphone.com";//[userDef stringForKey: @"proxyServer"];
        
        NSString *server = [userDef stringForKey: @"server"];
        ...
        if (!server || [server length] < 1)
            //server = @"sip.yelloworchidphone.com";
            [userDef stringForKey: @"server"];
        ...
        NSString *status =  [[NSUserDefaults standardUserDefaults] stringForKey:@"unregistered"];
        if([status isEqual:@"403"])
        {
            [window setRootViewController: [self applicationStartWithoutSettings]];
        }
        ...
        else
        {
            ...
            [window setRootViewController: [self applicationStartWithSettings]];
...
//{
//    NSLog(@"hiiiiiiiiiiiiiiiiiiifxghdytuyfuyfurtdyhtyydhdytdhtdytdtdytytdjnbkjnkjdnksdkfalkdfalfa");
//    [self sipStartup];
//}
//
...
-(void) dialup:(NSString *)phoneNumber number:(BOOL)isNumber
{
    ...
    AVAudioSession *audioSession=[[AVAudioSession sharedInstance]isOtherAudioPlaying]; // get your audio session somehow
   ...
   // BOOL success = [audioSession overrideOutputAudioPort:AVAudioSessionPortOverrideSpeaker error:&error];
    ...
    if (!hasMicro)
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"No Microphone Available", @"Dilsay SIP Client")
                                                        message:NSLocalizedString(@"Connect a microphone to phone", @"Dilsay SIP Client")
                                                       delegate:nil
                                              cancelButtonTitle:NSLocalizedString(@"OK", @"Dilsay SIP Client")
    ...
    if (!isConnected && [self wakeUpNetwork] == NO)
    {
        ...
        //else
        //  {
        // UIActionSheet *actionSheet = [[[UIActionSheet alloc] initWithTitle:NSLocalizedString(@"The SIP server is unreachable!",@"Adore SIP Client")
        //      delegate:self
        // cancelButtonTitle:NSLocalizedString(@"Cancel",@"Adore SIP Client")
        // destructiveButtonTitle:nil
        //  otherButtonTitles:NSLocalizedString(@"Cellular call",@"Adore SIP //Client"),
        // nil] autorelease];
